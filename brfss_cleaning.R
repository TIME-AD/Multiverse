...
library(haven)
mydir <- getwd()
brfss2023 <- read_xpt(paste0(mydir, "/LLCP2023.XPT"))
#colnames(brfss2023)
...
cohort <- cohort %>%
  rename(age_imputed = "_AGE80", # Imputed age value, collapsed above 80
         ...
         bingedrink = "_RFBING6", #binge drinker (>=5 for men, >=4 for women on one day)
         drink_perweek = "_DRNKWK2", #number of drinks per week 
         heavydrink = "_RFDRHV8", #heavy drinker (>14 per week for men, >7 for women)
         drink_past30 = ALCDAY4, #number of days per month with a drink 
         averagen_drink = AVEDRNK3, #average drinks per day 
...
saveRDS(cohort, paste0(mydir, "/cleaned_brfss.RDS"))
